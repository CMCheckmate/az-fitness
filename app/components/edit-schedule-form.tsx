...
export default function EditSchedules({ data, className }: { data: QueryResultRow, className?: string}) {
    ...
    return (
        <form action={dispatch} onSubmit={() => { setResponse('Loading...') }} className={`${className} table-row text-red-400 font-bold`}>
            ...
            <div className='table-cell border-2'>
                <input type='date' name='date' id='date' defaultValue={data.date_time.toISOString().split('T')[0]} className='p-2 text-center bg-transparent' required />
            </div>
            <div className='table-cell border-2'>
                <input type="time" name='time' id='time' defaultValue={data.date_time.toTimeString().split(' ')[0]} className='p-2 text-center bg-transparent' required />
            </div>
            <div className='table-cell border-2'>
                <input type='number' name='length' id='length' min={0} defaultValue={data.length} className='p-2 text-center bg-transparent' required />
            </div>
            <div className='table-cell border-2'>
                <textarea name='comments' id='comments' spellCheck={false} defaultValue={data.comments} className='p-4 align-middle text-center bg-transparent' />
            </div>
