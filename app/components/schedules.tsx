        ...
        <table className="w-3/4 min-w-max border-2">
          <thead className="border-2">
            <tr className="divide-x-2 bg-red-600">
              <th className="table-cell p-2 text-center text-l text-white font-bold">
                Number
              </th>
              <th className="table-cell p-2 text-center text-l text-white font-bold">
                Date
...
export default async function Schedules({
  ...
  const excludedDayTimes = {
    ...
    2: {
      ...
      excluded: [{ start: "13:30", end: "16:00" }],
    },
    3: {
      start: "06:00",
      end: "18:00",
    ...
    4: {
      ...
      excluded: [{ start: "11:30", end: "15:00" }],
    },
  ...
  async function getScheduleTimes(
    dateTime: { date: string; time: string } | null = null
  ) {
    const startTimes = {} as StartTimes;
    const scheduleTimes = {} as ScheduleTimes;

    for (const [date, exclusions] of Object.entries(
      Object.assign(await getExcludedTimes(dateTime), excludedDayTimes)
    )) {
      const day = /^[0-6]+$/.test(date)
        ? Number(date)
        : parse(date, "yyyy-MM-dd", new Date()).getDay();
      ...
      while (format(startTime, "HH:mm") <= excludedDayTimes[day].end) {
        ...
        const exclusionList =
          "excluded" in exclusions
            ? (exclusions.excluded as [])
            : exclusions.concat(excludedDayTimes[day].excluded);

        for (const exclusion of exclusionList) {
          if (
            format(addMinutes(startTime, scheduleInterval), "HH:mm") >=
              exclusion.start &&
            time <= exclusion.end
          ) {
    ...
    for (const [date, times] of Object.entries(startTimes)) {
      ...
      for (const startTime of times) {
        ...
        while (
          scheduleTimes[date][startTime].length <
          maxScheduleLength / scheduleInterval
        ) {
          ...
          if (
            !startTimes[date].includes(endTime) &&
            format(time, "HH:mm") <= excludedDayTimes[day].end
          ) {
            break;
          }
        }
        ...
        <div className="w-3/4 min-w-max table border-2">
          <div className="table-header-group">
            <div className="table-row divide-x-2 bg-red-600">
              <div className="table-cell p-2 text-center text-l text-white font-bold">
                Number
              </div>
              {session?.user.status == "Administrator" && (
                <div className="table-cell text-center p-2 text-l text-white font-bold">
                  Name
                </div>
              )}
              <div className="table-cell p-2 text-center text-l text-white font-bold">
